...
class CodeDefinition
{
    ...
    public function asHtml(ElementNode $el)
    {
        ...
        if($el->getTagName()==="url"){
			if(isset($GLOBALS['isInFrame'])){
				$html = preg_replace('/href="(.*)"/', 'style="color:#859ce6" target="_parent" href="'.$this->externalLink($el->getAttribute()).'"', $html);
			}else{
				$html = preg_replace('/href="(.*)"/', 'class="default" href="'.$this->externalLink($el->getAttribute()).'"', $html);
			}
		}
    ...
    private function externalLink($link){
        $url_parts=parse_url($link);
        if($url_parts['host']!='techberry.org'){
           $mysqli = new \mysqli(
            "techbmain.db.11499199.hostedresource.com",
            "techbmain",
            "H763#kjsduy93",
            "techbmain");
            $enableCheckLink=1;
            if($enableCheckLink==1){
                if($stmt = $mysqli->prepare("SELECT salt FROM links_external WHERE url = ? LIMIT 1")){
                    ...
                    $stmt->fetch();
                    if($stmt->num_rows()==1){
                        // link already present so use it
                        $stmt->close();
                        return 'http://techberry.org/url?src='.base64_encode($salt);
                    }else{
                        $stmt->close();
                        // new link, so add it
                        $salt = hash('sha512', uniqid(mt_rand(1, mt_getrandmax()), true));
                        if($stmt = $mysqli->prepare("INSERT INTO links_external (url,salt) VALUES(?,?)")){
                            $stmt->bind_param('ss',$link,$salt);
                            $stmt->execute();
                            $stmt->close();
                            return 'http://techberry.org/url?src='.base64_encode($salt);
                        }else{
