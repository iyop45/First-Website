    ...
    public function requestBadge($badge_name, $user_id)
    {
        if ($stmt = $this->mysqli->prepare("
			SELECT users.reputation,
				   CASE WHEN user_badges.accumulated IS NULL THEN 0 ELSE user_badges.accumulated END,
				   ( 
						SELECT	badges.id
									FROM badges
									WHERE badges.name = ?
			...
			(
				...
				AND user_badges.id_badge = ( SELECT id FROM badges WHERE badges.name = ? )
			)
			WHERE users.id = ? LIMIT 1"))
        {
            $stmt->bind_param('ssssi', $badge_name, $badge_name, $badge_name, $badge_name, $user_id);
            $stmt->execute();
			$stmt->store_result();
            $stmt->bind_result($reputation, $accumulated, $badge_id, $badgeName, $isStackable);
            $stmt->fetch();
			// If user already has badge and badge is stackable OR if user does not have badge
            if (($accumulated >= 0 && $isStackable == 1) || ($accumulated == 0 && $isStackable == 0))
            {
                // User has earnt the badge
                $notify_text          = "You've earnt the <a class='default' href='http://techberry.org/account/achievements/'>" . $badgeName . "</a> badge";
                $stmt->close();
