...
@define("CAPTCHA_IMAGE_SIZE", "310x60");  // randomly adapted a little
...
@define("CAPTCHA_FONT_DIR", dirname(__FILE__).'/fonts/captcha/');
...
class easy_captcha {
   ...
   function form($add_text="&rarr;&nbsp;") {
      ...
      $HTML =
         ...
         '<img style="padding:5px;background:white;display:block;border:1px solid #ddd;float:left;margin-bottom:5px;" src="'.$img_url .'&" width="'.$this->image->width.'" height="'.$this->image->height.'" alt="'.$alt_text.'" align="middle" '.$onClick.' title="' . CAPTCHA_REDRAW_TEXT . '" />' .
         '&nbsp;' .
         $add_text .
         '<input placeholder="Captcha" title="' . CAPTCHA_PROMPT_TEXT . '" type="text" '.$onKeyDown.' id="'.$p_input.'" name="'.$p_input.'" value="'.(isset($_REQUEST[$p_input])?htmlentities($_REQUEST[$p_input]):"") .
...
function captcha_check_solution() {
   ...
   if (len >= 4) {
      inf.style.border = "1px solid #FF9955";
   }
